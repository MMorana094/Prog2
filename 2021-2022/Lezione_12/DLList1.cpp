...
//Indici di testa e coda
	...
	public:
		...
		friend ostream& operator<< (ostream& os, Static_queue<T>& obj){
			...
			os << "\nSize= " << obj.size << endl;
			os << "\nMaxSize= " << obj.maxSize << endl;

			for(int i=obj.head, count=0; count < obj.size; count++){
				os << "\tArray[" << i << "] = " << obj.array[i] << endl;
				i = (i+1) % obj.maxSize; 				//Incremento i e ne faccio il modulo per essere sicuro di verificare la condizione di uscita
			}
